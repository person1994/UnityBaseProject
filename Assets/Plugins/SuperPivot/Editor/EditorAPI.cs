    ...
    public class EditorAPI
    {
        ...
        void WrapSelectedTransforms()
        {
            ...
            for (int i = 0; i < selected.Length; ++i)
            {
                ...
                wrapper.position = towrap.position;
                wrapper.rotation = towrap.rotation;
                wrapper.localScale = towrap.localScale;
        ...
        public void DrawGUI(Transform[] selectedTransforms)
        {
            if (targetManager.isEmpty)
            {
                bool needToWrap = !API.CanChangeAtLeastOnePivot(selectedTransforms);
