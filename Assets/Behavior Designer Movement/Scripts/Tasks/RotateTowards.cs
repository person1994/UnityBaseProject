...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    {
        public SharedTransform baseTransform;
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (Quaternion.Angle(BaseTransform.rotation, rotation) < rotationEpsilon.Value) {
                return TaskStatus.Success;
            }
            // We haven't reached the target yet so keep rotating towards it
            BaseTransform.rotation = Quaternion.RotateTowards(BaseTransform.rotation, rotation, maxLookAtRotationDelta.Value);
        ...
        private Quaternion Target()
        {
            ...
            var position = target.Value.transform.position - BaseTransform.position;
            if (onlyY.Value) {
                position.y = 0;
            }
            if (usePhysics2D) {
